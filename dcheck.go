...
func GenerateDatasetSummary(ds MeasurementDataset) string {
    ...
    s += fmt.Sprintf("Gaps >1w:   %.0f%% (%d)\n", float32(ds.GapsGt1week)/float32(ds.Measurements), ds.GapsGt1week)
    s += fmt.Sprintf("Gaps >1d:   %.0f%% (%d)\n", float32(ds.GapsGt1day)/float32(ds.Measurements), ds.GapsGt1day)
    s += fmt.Sprintf("Gaps >12hr: %.0f%% (%d)\n", float32(ds.GapsGt12hr)/float32(ds.Measurements), ds.GapsGt12hr)
    s += fmt.Sprintf("Gaps >6hr:  %.0f%% (%d)\n", float32(ds.GapsGt6hr)/float32(ds.Measurements), ds.GapsGt6hr)
    s += fmt.Sprintf("Gaps >2hr:  %.0f%% (%d)\n", float32(ds.GapsGt2hr)/float32(ds.Measurements), ds.GapsGt2hr)
    s += fmt.Sprintf("Gaps >1hr:  %.0f%% (%d)\n", float32(ds.GapsGt1hr)/float32(ds.Measurements), ds.GapsGt1hr)
    s += fmt.Sprintf("Gaps >30m:  %.0f%% (%d)\n", float32(ds.GapsGt30m)/float32(ds.Measurements), ds.GapsGt30m)
